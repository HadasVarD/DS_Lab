...
### Embedding the train and the test data sets
...
![image](image.png){width="200," fig-align="center"}
...
we took 49 sentences generated with chatGPT which express high arousal original prompt was.
...
main.indxTrain <- createDataPartition(y = main.knn$label, p = 0.7, list = FALSE)
...
confusionMatrix(main.test$knn, main.test$label, positive = "1")
```
...
confusionMatrix(glm_pred, test_vec2$label)
```
...
confusionMatrix(boost_pred, reference = test_vec2$label, positive = "yes")
```

Surprisingly, this model performed worse than the simpler models.
