...
we decided to keep exploring the data based on those two dimensions.

### Clustering texts along the valence dimention: postive and negative
...
### Create vector that present Arousal dimension

we took 49 sentences generated with chatGPT which express high arousal. For example:
...
We trained a machine learning model using k-means clustering that relies on two dimensions with hopes to partition the texts into two distinct clusters.
...
```{r}
#| panel: sidebar
selectInput(inputId = "model", label = "Classifier",
            choices = c("K-Nearest Neighbors", "Logistic Regression", "Boosted Decision Trees"), selected = "K-Nearest Keighbors")

selectInput(inputId = "measures", label = "Performance Measure",
            choices = c("Accuracy", "Kappa", names(knn_con_mat$byClass)),
            selected = "Kappa", multiple = T)
```
...
knn_con_mat <- read_rds("knn_con_mat.rds")
glm_con_mat <- read_rds("glm_con_mat.rds")
boost_con_mat <- read_rds("boost_con_mat.rds")

classifier_performance <- function(model, measures) {
  
  if (model == "K-Nearest Neighbors") {con_mat <- knn_con_mat}
  if (model == "Logistic Regression") {con_mat <- glm_con_mat}
  if (model == "Boosted Decision Trees") {con_mat <- boost_con_mat}
  
  model_measures <- c(con_mat$overall["Accuracy"], con_mat$overall["Kappa"], con_mat$byClass)
  
  return(data.frame("Measure" = measures,
                    "Value" = model_measures[measures]))
}

models_selected <- reactive({input$model})
measures_selected <- reactive({input$measures})

output$table1 <- renderTable(classifier_performance(models_selected(), measures_selected()))
```
